...
export default function App() {
  ...
  const [showingCreateNewPostForm, setShowingCreateNewPostForm] =
    useState(false); //Default value is false
  ...
  function renderPostsTable() {
    return (
      <div className="table-responsive mt-5">
        ...
        <button
          onClick={() => setPosts([])}
          className="btn btn-dark btn-ls w-100"
        >
  ...
  function onPostUpdated(updatedPost){
    ...
    //make a copy of the posts array, persist the updates to the post in array, instead of doing another API call 
    //copy array in array "postsCopy"
    let postsCopy = [...posts];
    ...
    setPosts(postsCopy);
    //interpolated string
    alert(`Post successfully updated. After clicking OK, look for the post with the title "${updatedPost.title}" in the table below to wee the updates.`)
  }
